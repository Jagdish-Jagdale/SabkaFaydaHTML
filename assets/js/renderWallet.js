    ...
    walletData.forEach(item => {
        ...
        <article class="wallet-history-card" style="cursor: pointer;" onclick="location.href='order-breakdown.html'">
            ...
            <div class="ps-4 text-end flex-grow-1 flex-md-grow-0">
                <button class="btn btn-link text-primary fw-bold text-decoration-none p-0" style="font-size: 0.85rem;" onclick="event.stopPropagation(); viewTxDetails('${item.orderId}', '${item.date}, ${item.time}', '${item.title}', '₹ ${item.amount}', '${item.points}')">View Details</button>
            </div>
        </article>`;
    });
